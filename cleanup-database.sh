...
# ============================================
# Скрипт очистки базы данных (кроме админа)
# ============================================

set -e

echo "🗑️  Скрипт очистки базы данных Tyrian Trade"
echo "============================================"
echo ""
echo "⚠️  ВНИМАНИЕ: Эта операция удалит ВСЕХ пользователей кроме админа!"
echo "   - Все посты будут удалены"
echo "   - Все комментарии будут удалены"
echo "   - Все подписки будут удалены"
echo "   - Все OAuth данные будут удалены"
echo ""
...
if [ "$1" == "production" ] || [ "$1" == "prod" ]; then
    ...
    # Загружаем production переменные из custom-backend/.env
    ...
    echo ""
    echo "Использование:"
    echo "  ./cleanup-database.sh local       # Для локальной базы данных"
    echo "  ./cleanup-database.sh production  # Для production базы данных"
    echo ""
    exit 1
fi
...
USER_COUNT=$(PGPASSWORD=$DB_PASSWORD psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -t -c "SELECT COUNT(*) FROM users WHERE username != 'admin' AND role != 'admin';" | xargs)
...
# Подтверждение
echo "⚠️  Вы уверены, что хотите удалить $USER_COUNT пользователей?"
