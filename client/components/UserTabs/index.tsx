...
const UserTabs: FC<Props> = ({ isOwn = true, viewMode = "normal", effectiveCategories = [], monetizationFilter = "all" }) => {
  ...
  const renderEmptyState = (tabId: TabId) => (
    <div className="flex flex-col items-center justify-center gap-3 rounded-3xl border border-[rgba(24,27,34,0.95)] bg-[rgba(12,16,20,0.40)] p-6 text-center backdrop-blur-[50px]">
  ...
  const renderPostWithBadges = (post: FeedPostProps, index: number) => {
    ...
    return (
      <article
        ...
        className="mx-auto flex w-full max-w-full flex-col gap-3 rounded-3xl border border-[rgba(24,27,34,0.95)] bg-background p-5 shadow-[0_24px_60px_-35px_rgba(0,0,0,0.65)] backdrop-blur-[32px] transition-colors duration-200 hover:border-[#A06AFF]/60"
      >
        ...
        <div className="flex items-center justify-between border-t border-[rgba(24,27,34,0.95)] pt-3 text-xs text-[#8E92A0]">
          ...
          <button
            ...
            className={`group flex items-center justify-center gap-2 rounded-[32px] px-2.5 py-3 text-[15px] font-bold transition backdrop-blur-[58px] ${
              activeTab === tab.id
                ...
                : "border border-[rgba(24,27,34,0.95)] bg-[rgba(12,16,20,0.50)] text-[#B0B0B0] hover:border-[#2F3240] hover:bg-[rgba(18,22,28,0.8)] hover:text-white"
            }`}
          >
            <span
