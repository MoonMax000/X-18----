        ...
        return (
          <div className="flex flex-col flex-1">
            <div className="flex flex-col gap-4">
              {/* TEMPORARY TEST BUTTON - Easy to click for testing */}
              <button
                onClick={handleLogin}
                disabled={isBlocked || isLoading}
                className={cn(
                  "w-full py-4 flex items-center justify-center gap-2 rounded-xl text-lg font-bold text-white transition-all duration-300 mb-4",
                  isBlocked || isLoading
                    ? "bg-gray-600 cursor-not-allowed opacity-50"
                    : "bg-gradient-to-r from-green-500 to-emerald-600 hover:shadow-xl hover:shadow-green-500/40 hover:scale-[1.02] active:scale-[0.98]"
                )}
              >
                {isLoading ? (
                  <>
