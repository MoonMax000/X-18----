...
const NewNavBar: FC<Props> = ({ variant = 'primal', isOpen = false, onClose }) => {
  ...
  const renderElement = (el: NavElementProps, isMobile: boolean = false) => {
    if (el.children && el.children.length > 0) {
      ...
      return (
        <div key={el.title}>
          <div className="relative group/parent-tooltip">
            <button
            ...
            {/* Tooltip for collapsed state - only shows on button hover */}
            {isCollapsed && !isMobile && (
              <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-[#1E1E1E] text-white text-sm font-medium rounded-lg opacity-0 invisible group-hover/parent-tooltip:opacity-100 group-hover/parent-tooltip:visible transition-all duration-200 pointer-events-none whitespace-nowrap z-50 shadow-lg border border-[#2A2A2A]">
                {el.title}
              </div>
            )}
          </div>
