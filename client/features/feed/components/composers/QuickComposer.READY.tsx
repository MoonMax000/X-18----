...
type Props = { 
  ...
  onPostCreated?: () => void; // NEW: Callback after successful post
};

export default function QuickComposerReady({ onExpand, onPostCreated }: Props) {
  const { toast } = useToast(); // NEW: For notifications
  ...
  const {
    ...
    insertCodeBlock,
    removeCodeBlock,
    reset, // NEW: Reset form after post
  } = useSimpleComposer();
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 mb-[-1px]">
        <Textarea
          ...
          disabled={isPosting} // NEW: Disable while posting
        />
        ...
        {media.length > 0 && (
          <MediaGrid
            ...
            readOnly={isPosting} // NEW: Readonly while posting
          />
        )}
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <ComposerFooter
            ...
            onPost={handlePost}
            isPosting={isPosting} // NEW: Show loading state
          />
        </div>
